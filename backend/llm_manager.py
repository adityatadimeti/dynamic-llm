...
class LLMManager:
    def __init__(self, llms, current_index=0):
        self.llms = llms
...
def process_through_llm(llm_manager, good_model, cur_index, chat_history, user_input):
    print(" in llm func")
    llm = llm_manager.get_next_llm(good_model, cur_index)
    client = llm.client
    model_name = llm.model_name
    ...
    try:
        ...
        return {
            ...
            'pricing': get_pricing(model_name, user_input)
        }
        # was_good_model = input("Was the model response good? (y/n): ")
        # while was_good_model != "y" and was_good_model != "n":
        #     print("Invalid input. Please enter 'y' or 'n'.")
